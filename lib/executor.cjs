...
const EXECUTORS = {
  ...
  devops: {
    ...
    instructions: [
      ...
      '+ a green suite exist, then authorize the push by setting WRXN_ACTIVE_AGENT to devops in .claude/settings.local.json (an inline command-scoped assignment never reaches the gate hook), then push. This is the single path through the push gate.',
    ],
