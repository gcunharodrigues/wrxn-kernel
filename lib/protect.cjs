 ...
 * `https://host/owner/repo.git`, with or without the `.git` suffix / a trailing slash). Returns null
 * when no slug can be read — protection then fail-soft-skips (a non-GitHub or malformed remote).
 */
