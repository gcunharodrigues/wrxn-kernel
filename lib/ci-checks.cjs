...
// selects which managed files were laid. `.mcp.json` is the one exception: it is managed but
...
function managedIntegrity(root, opts) {
  ...
  const managed = manifest.files.filter((f) => f.class === 'managed' && f.path !== MCP_PATH);
  const failures = [];
  let checked = 0;
  for (const f of managed) {
    ...
    // Present on disk → it must byte-match the kernel source REGARDLESS of the claimed profile, so a
    // flipped receipt profile cannot hide a tampered workspace-managed file (CF-2 defense-in-depth).
    checked++;
